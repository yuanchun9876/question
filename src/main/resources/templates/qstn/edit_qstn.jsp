								...
								<div class="box box-primary">
									...
									<div class="box-body">
										<div class="form-group">
											...
											<div class="col-sm-10">
											   <textarea id="editor" type="text/plain"   hidden="true" autofocus>${qstn.qstnTitle }</textarea>
											...
											<div class="col-sm-4">
												<select class="form-control" name="qstnTypeId">
													...
													<c:forEach items="${qstnTypeList }" var="qstnType">
														<option value="${qstnType.qstnTypeId }"
															${qstnType.qstnTypeId==qstn.qstnTypeId?"selected":"" }
														>${qstnType.qstnTypeName }</option>
													</c:forEach>
												</select>
											</div>
											...
											<div class="col-sm-4">
												<select class="form-control" name="qstnFromTypeId">
													...
													<c:forEach items="${qstnFromList}" var="qstnFrom">
														<option value="${qstnFrom.qstnFromTypeId }" 
															${qstnFrom.qstnFromTypeId==qstn.qstnFromTypeId?"selected":"" }
														>${qstnFrom.qstnFromTypeCode }</option>
													</c:forEach>
												</select>
											</div>
										</div>
											...
											<div class="col-sm-3">
												<select class="form-control"  onchange="selectSubj(this.value)" >
													...
													<c:forEach items="${subjList}" var="subj">
														<option value="${subj.subjId }"
															${subj.subjId == subjId?"selected":"" }
														>${subj.subjTitle }</option>
													</c:forEach>
												</select>
											</div>
												...
												<select class="form-control" onchange="selectUnit(this.value)"  id="unitSelect">
													...
													<c:forEach items="${subjUnitList}" var="unit">
														<option value="${unit.subjUnitId }"
															${unit.subjUnitId == subjUnitId?"selected":"" }
											...
											<div class="col-sm-3">
												<select class="form-control" name="subjSctnId" id="sctnSelect">
													...
													<c:forEach items="${subjSctnList}" var="subjSctn">
														<option value="${subjSctn.subjSctnId }" 
															${subjSctn.subjSctnId==qstn.subjSctnId?"selected":"" }
														>${subjSctn.subjSctnTitle }</option>
													</c:forEach>
												</select>
											</div>
										</div>
										<div class="form-group">
											...
											<div class="col-sm-10">
											   	<textarea id="editor2" type="text/plain"   hidden="true" autofocus>${qstn.qstnPictext }</textarea>
